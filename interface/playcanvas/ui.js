...
Ui.prototype.show = function(description, artist, social, workshop, title, link) {
    ...
    document.querySelector('#ui-workshop').innerHTML = workshop || 'Open Call'
    document.querySelector('#ui-title').innerHTML = link ? `<a id="ui-target-blank" href="${link}" target="_blank">${title}</a>` : title
    ...
    switch(workshop) {
        ...
        case 'Nature Patterns In AR':
            ...
            break
        default:
            document.querySelector('#ui-workshop').href = 'https://www.captcha-mannheim.de/#/open-call'
    }
...
Ui.prototype.initHtml = async function() {
    ...
    var space = window.location.pathname.replaceAll('/', '')
